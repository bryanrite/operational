module Operational
  class Operation
    module Contract
      def self.Build(name: :contract, contract:, model_key: nil, model_persisted: false, prepopulate_method: :prepopulate)
        lambda do |state|
          state[name] = contract.new
          state[name].instance_variable_set(:@_operational_model_persisted, model_persisted)
          ...
          if model_key.present?
            ...
            state[name].instance_variable_set(:@_operational_model_persisted, state[model_key].persisted?)
          end
      ...
      def self.Validate(name: :contract, params_path: nil)
        lambda do |state|
          valid_path = case
            ...
            when params_path.is_an?(Array) then params_path
          end
